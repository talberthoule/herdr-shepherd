...
const PHASE_TITLE={attempted:'Wrapper is about to run the coordination command.',succeeded:'Wrapper submitted the message. Delivery is unconfirmed until a pane read shows the target processing it.',failed:'Wrapper reported a non-zero exit.'};
const DELIVERY_TITLE={confirmed:'Target was idle before the send and started working after it, so the submit landed.',unconfirmed:'Target was idle before the send and did not start within the wait window. Pane read before resending; on the keystroke transport, suspect a stuck composer.',queued:'Target was already working, so the message queued behind the running turn. Pane read later to confirm it surfaced.',unknown:'Status probe failed, so delivery could not be assessed.'};
